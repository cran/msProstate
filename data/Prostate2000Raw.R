...
dbname <- ifelse(is.R(), paste(system.file("data", package="msProstate"), "Prostate2000Raw.db", sep="/"), "Prostate2000Raw.db")	
...
Prostate2000Raw <- list(mz=mz[,1], intensity=as.matrix(intensity), type=as.factor(type[,1]))
attr(Prostate2000Raw, "class") <- "msSet"
attr(Prostate2000Raw, "data.name") <- "Prostate Cancer 2000 Raw"
dbDisconnect(con)
